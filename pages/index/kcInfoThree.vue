<template>
	<view class="Two">
		<u-navbar :title="topInfo.kcmc || topInfo.kc_mc" back-icon-color="#333" :border-bottom="false" ></u-navbar>
		<view class="zsjj-wrapper">
			...
			<view class="zsjj-desc">
				<view class="zsjj-bt">
					<view class="lx" :class="{'color': topInfo.lxmc == '素养'}">{{topInfo.lxmc}}</view>
					{{topInfo.kcmc || topInfo.kc_mc}}
				</view>
				<view class="zsjj-qt">
					<view>
						<image src="../../static/images/class_bf.png"></image>
						<text>讲师{{topInfo.kc_wksl}}</text>
					</view>
					<view>
			...
			<view class="boder-line"></view>
		</view>
		<u-tabs  :list="flList" name="mc" font-size="32" inactive-color="#999999" active-color="#333333" :bar-style='barStyle' :is-scroll="false" :current="current" @change="change"></u-tabs>
		...
		<view style="padding:20rpx 30rpx 110rpx;" v-if="current == 0">
			<view class="ins-wrapper" v-for="(items, i) in kcInfo" :key="i">
				...
				<view class="ins-list" v-for="(item, index) in items.lc" :key="index">
					<!-- item.sfmf == 1 && topInfo.kc_sfsf == 1 && shzt == 0 && is == false && userInfo.xssfvip == 0 -->
		...
		<view style="padding:20rpx 30rpx 110rpx;" v-if="current == 1">
			<view>
				<image v-if="topInfo.kc_ts" :src="topInfo.kc_ts" mode="widthFix" style="width: 100%;"></image>
				<!-- <rich-text :nodes="topInfo.kc_sydx" v-if="topInfo.kc_sydx"></rich-text> -->
				<u-empty text="暂无内容" mode="list" v-else margin-top="150"></u-empty>
			</view>
		</view>
		
		 <!-- topInfo.kc_sfsf == 1 && shzt == 0 && userInfo.xssfvip == 0 && is == false && topInfo.xj != 0 -->
		<view class="pay-box" v-if="shzt == 0 ?
									 userInfo.xssfvip == 0 ?
									 is == false ?
									 topInfo.kc_sfsf == 1 ? true
									 :false
									 :false
									 :false
									 :false">
			<view class="je">￥<text>{{topInfo.xj}}</text></view>
...
<script>
	export default {
		...
		computed:{
			desc(){
				if(this.topInfo.kc_sydx ){
					this.topInfo.kc_sydx = this.topInfo.kc_sydx.replace(/<img/g, '<img style="width: 100%; display:block;"')
					this.topInfo.kc_sydx = this.topInfo.kc_sydx.replace(/<p/g, '<p style="font-size: 15px; color: #777;line-height: 23px;"')
					this.topInfo.kc_sydx = this.topInfo.kc_sydx.replace(/<video/g, '<video style="width: 100%;"')
				}
				return false
			}
		},
		methods:{
			...
			iosPay(){
				...
				let obj = {
					spmc:this.topInfo.kc_mc,
					gmje:this.topInfo.xj,
					ddlx:1  ,//1课程 0x学习卡
					spxj:this.topInfo.xj,
					yhlx: 0 ,  // 0无 1 卷 2 码
					yhje:0,
					xsid:getApp().globalData.userInfo.xsid,
					fkfs:0,      //1zfb 0 wx
					spid:this.kcid,
					kcfm:this.topInfo.kc_fm,
					spyj:this.topInfo.yj
				}
			...
			isFlag(){
				...
				let kcsfsf = this.topInfo.kc_sfsf
				if(this.shzt == 0){
					//不在审核中
					if(kcsfsf == 1){
						//收费
			...
			handelSP(item){
				...
				let lcid = item.lcid
				let sfmf = item.sfmf
				let isVip = this.userInfo.xssfvip
				let ddList = this.userInfo.dingdan
				if(this.flag || sfsf != 1){
