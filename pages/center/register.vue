<template>
	<view>
		...
		<view class="center-box">
			<image src="../../static/zuce.png" class="title" mode="widthFix"></image>
			<u-input  :custom-style="inputbg"  placeholderStyle="color:#ffffff" :clearable="false" v-model="form.name"  placeholder="请输入姓名" type="text"/>
			...
			<u-input  :custom-style="inputbg" @click="njShow = true"  placeholderStyle="color:#ffffff" :clearable="false" v-model="njmc" disabled  placeholder="请输入手机号" type="text"/>
			<u-button @click="submit" :ripple="true" ripple-bg-color="#ffffff" shape="circle" class="submit">注册</u-button>
		</view>
		<u-toast ref="uToast" />
...
<script>
	...
	export default {
		...
		methods:{
			...
			getNJList(){
				this.$u.post('app/appLoginController/selectNj').then(res=>{
					...
					if(res){
						...
						this.$refs.uToast.show({title:res})
					}
				}).catch(err=>this.$refs.uToast.show({title:err}))
			},
			goBack(){
				uni.navigateBack()
			},
			...
			submit() {
				if(this.form.tel == '' || this.form.password == '' || this.form.yzm == '' || this.form.name == ''){
					this.$refs.uToast.show({
						title:'请补全信息',
						type: 'error',
					})
			...
			handleData(){
				this.$u.post('app/appLoginController/bs_addXSInfo',{
					xsmc:this.form.name,
					xsdh:this.form.tel,
